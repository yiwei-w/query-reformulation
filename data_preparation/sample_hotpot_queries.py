...
df = pd.read_csv('hotpot_requests.csv')
batch_size = 10
data = []
MAX_RETRY = 4
...
def generate_queries_batch(batched_requests, retry_count=0):
    try:
        ...
        if retry_count < MAX_RETRY:
            ...
            dummy_data = [{"inner_monologue": "dummy_monologue", "search_query": ["dummy_query"]} for _ in batched_requests]
            return dummy_data
...
df.to_csv('hotpot_requests_and_queries.csv', index=False)
